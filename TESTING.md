...
- [ ] **Tested on a real device: the close confirmation did not appear.** The user tested on a local-user device where the only admin account was the Built-in Administrator, launching an already-elevated PowerShell (signed in as Built-in Administrator directly, not a UAC prompt from a standard user's own session) and running the `irm .../Deploy.ps1 | iex` bootstrap from inside it. Closing the window afterward just closed normally, with no delete confirmation. Root cause not yet confirmed -- possibilities include a stale GitHub Raw CDN cache serving an older `Deploy.ps1` at the moment of testing (the fix commit was pushed, but `raw.githubusercontent.com` can cache for a few minutes after a push), since `Request-Administrator` returning early when already elevated (`$SystemInfo.IsAdministrator` true from the start, no relaunch needed) was traced through the code and should still correctly carry `$DeleteOnClose` from `Start.ps1`'s own parameter binding through to `Show-MainWindow` with no relaunch involved. Needs a clean retest (the user was asked to re-run the bootstrap and retest) before concluding whether this is a stale-cache false alarm or a real, still-unfixed bug in the already-elevated code path specifically.
