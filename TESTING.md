...
- Remote bootstrap fix for the "Could not find a part of the path" `Move-Item` failure reported on some devices -- needs re-confirming on a device that previously failed this way
- WinGet availability fix (`Test-WingetAvailable`, the AppX re-registration fallback) for devices elevated as a different account than the real interactive user -- needs confirming on a device that previously showed WinGet as red/unavailable
...
## Fixed: WinGet Status Showed Red on Devices Elevated as a Different Account

Reported by the user on some devices, not others: the system info bar's WinGet indicator showed red (unavailable) even though the device genuinely had WinGet installed and working for its real user.

- [x] **Root cause, matching a pattern already found twice this session (the download-path and logged-user bugs)**: `Test-Winget`/`Test-WingetPackage` both called `Get-Command -Name "winget.exe"`, which only ever resolves for the *current process's own account*. `winget.exe` is registered as a per-user app execution alias (`<profile>\AppData\Local\Microsoft\WindowsApps\winget.exe`) -- when elevation switched to a different admin account than the real interactively logged-on user (e.g. the Built-in Administrator, required when the real device user is a standard, non-admin account), that account genuinely does not have the alias on its own PATH, even though the real user's own session has WinGet working fine.
- [x] **This was more than a cosmetic status-dot bug**: `Invoke-WingetCommandWithSourceRepair` calls `& winget @WingetArguments` directly, which relies on the exact same per-account PATH resolution -- meaning real WinGet installs would have silently failed on these same devices too, not just the indicator. Fixing only the colored dot would have been actively misleading.
- [x] Considered and rejected resolving the real interactive user's own alias *file path* directly (the same `Win32_ComputerSystem` -> `Win32_UserAccount` -> `Win32_UserProfile` chain `Deploy.ps1` already uses for its download-path fix): app execution aliases are thin stub launchers that activate the package for the *calling* process's own account session, not the file's original owner, so finding the file would not have proven it can actually run under this account.
- [x] **Fixed using a mechanism already proven working in this exact codebase** rather than an invented one: generalized `Repair-WingetSourceDataPackage`'s re-registration logic into `Repair-AppxPackageRegistration -PackageName <name>` (`Modules\Installation\WingetInstaller.ps1`), then added `Test-WingetAvailable`, which falls back to checking `Get-AppxPackage -AllUsers "Microsoft.DesktopAppInstaller"` (confirming the package is installed machine-wide, just not registered for this session) and re-registers it with the same `Add-AppxPackage -DisableDevelopmentMode -Register` call `Repair-WingetSourceDataPackage` already uses for a different known WinGet bug, before concluding WinGet is genuinely unavailable. `Test-Winget` (`SystemChecks.ps1`) and `Test-WingetPackage` now both delegate to this one function; `Install-ApplicationWithWinget`/`Uninstall-ApplicationWithWinget` now check it explicitly before ever calling `& winget`, returning a clean `Failed` result with a clear message instead of letting an unresolvable command throw a raw `CommandNotFoundException` into the queue.
- [x] Verified what's verifiable from this non-elevated dev environment: `.\Start.ps1 -ValidateOnly` passes; `Test-WingetAvailable` correctly returns `True` via the fast path on this machine (where WinGet is normally on PATH), confirming no regression for the common, unaffected case. `Get-AppxPackage -AllUsers` itself requires elevation to succeed (confirmed directly -- it throws `UnauthorizedAccessException` from this non-elevated shell), so the actual fallback/repair path could not be exercised here.
- [ ] **Not yet confirmed on a real device that showed this bug.** The repair mechanism is the same one already proven to work for a different WinGet issue, but the specific scenario this targets (WinGet installed machine-wide, not registered for the current elevated-as-a-different-account session) has not been reproduced or re-tested end-to-end against an actual affected device.
...
## Fixed: Remote Bootstrap Failed to Launch on Some Devices ("Could not find a part of the path")

Reported by the user: the bootstrap worked on some devices but failed on others with `Move-Item : Could not find a part of the path` immediately after "Extracting...", followed a moment later by `The argument '...\Start.ps1' to the -File parameter does not exist.`

- [x] **Root cause traced directly from the reported console output, not guessed at**: `Move-Item` (`Deploy.ps1`) had no `-ErrorAction Stop`, so its default non-terminating error let the script print "Ready at ..." and "Launching..." and reach the final `powershell.exe -File ...\Start.ps1` line regardless of whether the move actually succeeded -- exactly matching the reported sequence (the Move-Item error, then "Ready"/"Launching", then a second, more confusing failure about a missing `Start.ps1` that was never actually placed there because the first failure was silently swallowed). `Move-Item` also never creates missing intermediate folders, so if the target user's `<profile>\Desktop` did not already exist on the failing device (a broken OneDrive Known Folder Move redirection, or a profile whose standard folders were never fully provisioned, are the most likely reasons this would happen on some devices and not others -- not confirmed against the actual failing device, since it was not accessible directly, but consistent with every detail in the reported output), the move would fail with exactly this `DirectoryNotFoundException` message.
- [x] Fixed by creating the destination's parent folder first if missing (`New-Item -ItemType Directory` on `Split-Path $DestinationRoot -Parent`), and adding `-ErrorAction Stop` to `Move-Item` so any future failure stops the script immediately with a clear error instead of silently continuing into a broken launch.
- [x] Verified the corrected script still parses cleanly (`[System.Management.Automation.Language.Parser]::ParseFile`); the actual missing-Desktop-folder scenario could not be reproduced or re-tested against a real failing device from this session.
- [ ] Not yet confirmed on an actual device that previously failed this way. If it fails again after this fix, the new `-ErrorAction Stop` should at least surface a clear, specific error at the real point of failure instead of the confusing two-error sequence originally reported.
...
- [x] Live click-through of the real, unmodified `Show-MainWindow` confirmed this screen loads and its Refresh button works: navigating to the Temp Cleanup tab and clicking Refresh both completed with no error, as part of the broader GUI-modularization refactor's own end-to-end verification (see "Refactored the GUI Modules for Separation of Concerns" below).

---

## Refactored the GUI Modules for Separation of Concerns

Per the user's own request, feeling "the code in one file is too much now." A line-count survey confirmed three concrete, well-evidenced problems rather than just large files in the abstract: `Show-GuiCompletionModal`/`Get-GuiCompletionSummary`/`Invoke-GuiCopyCompletionSummary` (generic completion-reporting UI already reused by Temp Cleanup) were living inside `GuiApplicationsScreen.ps1`; `GuiWindowsConfigScreen.ps1` (864 lines) held three distinct screens (Windows Setup, Device Details, Asset ID) joined only by one shared background loader; and `Show-MainWindow` in `GuiWindow.ps1` was a 671-line god-function doing every screen's `FindName` and click-handler wiring in one body. `MainWindow.xaml` (1073 lines, the single largest file) was deliberately left alone, per the user's own choice, since splitting it would mean inventing a new runtime XAML-merge mechanism this app doesn't have, for a file that's markup rather than logic.

- [x] **Phase 1**: extracted the completion-modal trio into a new `Modules\Gui\GuiCompletionModal.ps1`, added to `$ModulePaths` before both `GuiApplicationsScreen.ps1` and `GuiTempCleanupScreen.ps1` (both call it). No call sites changed. Verified via `.\Start.ps1 -ValidateOnly` (51 modules).
- [x] **Phase 2**: split `GuiWindowsConfigScreen.ps1` into `GuiWindowsConfigShared.ps1` (the one shared CIM/BIOS/`Sample.txt` background loader, the full-refresh orchestrator, and the generic `{Status,Message}`-to-dialog mapper `Show-GuiResultDialog`), `GuiWindowsSetupScreen.ps1` (Rename/Local User/Power Settings), `GuiDeviceDetailsScreen.ps1` (the clipboard summary), and `GuiAssetIdScreen.ps1` (Lenovo WinAIA fields); deleted the original file. `Start.ps1`'s module list updated (54 total) in that order (shared first). No XAML or call-site changes needed, since PowerShell resolves functions by name regardless of which file defines them once everything is dot-sourced. Verified via `-ValidateOnly`.
- [x] **Phase 3**: decomposed `Show-MainWindow` into a thin orchestrator (XAML/font load, the handful of genuinely cross-screen pieces -- nav arrays, the completion modal's Ok/Copy buttons, the system info bar, title-bar/window-state/closing handlers -- and the one shared Windows Setup/Device Details/Asset ID Refresh wiring, since that spans controls owned by all three) plus one new `Initialize-GuiXScreen` function per screen, added to that screen's own existing file, each doing that screen's own `FindName` and click-handler wiring and returning its Nav Border/Text/Icon triple (and, for the three shared-report screens, whatever the Refresh wiring needs from them). Verified via `-ValidateOnly` (still 54 modules, since no new files were added in this phase).
- [x] **Caught and fixed a real, serious regression during Phase 3's own end-to-end verification, before it could ship**: calling each `Initialize-GuiXScreen` function normally (`$ApplicationsScreen = Initialize-GuiApplicationsScreen -Window $Window ...`) creates a new child scope for that function that is torn down the moment it returns -- since every plain (non-`GetNewClosure()`'d) click handler wired *inside* that function closes over variables in *that* scope, every single one of them (nav clicks, Select All, Refresh, checkbox toggles, across every screen) failed the instant it fired, with `"Cannot bind argument to parameter '...' because it is null"`. This is the same underlying rule already documented elsewhere in this file (an earlier attempt to swap `Show-MainWindow`'s own `ShowDialog()` for a non-blocking `Show()` tore down its scope the same way) -- what's new here is that it applies to *any* function returning, not just `Show-MainWindow` itself returning, which is exactly what Phase 3's design introduced. Fixed by dot-sourcing each call (`. Initialize-GuiApplicationsScreen -Window $Window ...`) instead of calling it plainly, which runs the function's body directly in `Show-MainWindow`'s own scope (no child scope created, nothing torn down), so every closure it wires now survives for the life of the window -- identical in shape to how the original, unrefactored code always worked. This was caught by *actually running the real, refactored `Show-MainWindow`* end-to-end (off-screen, background STA runspace, real 7-screen XAML, real click simulation) rather than trusting `-ValidateOnly`'s syntax-only check, and would otherwise have shipped a GUI where every single button was silently broken.
- [x] **End-to-end verification method**: ran the real, unmodified `Show-MainWindow` off-screen (`Left/Top = -6000`, `ShowInTaskbar = $false`) on a background STA runspace so `ShowDialog()` still blocks exactly like production, with `Show-GuiDialog` redirected to log instead of opening a real (blocking) dialog. Verification and simulated clicks ran from a `DispatcherTimer` created on the *same* thread as the window (fires on that thread's own message loop while `ShowDialog()` is blocking it) -- an earlier version of this harness tried to poll the still-running background runspace's `SessionStateProxy`/`Streams` from the outer script's own thread instead, which deadlocked outright (a busy runspace cannot safely be introspected from another thread), and `Write-Output` from inside a WPF-dispatched callback turned out not to reach `BeginInvoke`'s captured output stream either, so results were logged to a plain file instead of relying on either. Confirmed clean end-to-end: the window loads, all 7 nav items resolve with the correct labels, `$script:GuiWindowsConfigDeviceFields`/`$script:GuiAssetIdFieldTextBoxes`/the 3-strong shared-refresh-button array all have the expected shape, clicking every one of the 7 nav items switches screens correctly with zero errors, a real Select All click on Applications correctly selected all 30 catalog entries, and a real Temp Cleanup Refresh click completed cleanly.
- [x] **A second, narrower hang was found and correctly attributed to the test itself, not the code**: simulating a click on `RefreshWindowsSetupButton` specifically (as opposed to every other button, which worked) hung indefinitely. Root cause: `Start-GuiWindowsConfigLoad` deliberately disables all three Windows Setup/Device Details/Asset ID Refresh buttons for the duration of its background load and only re-enables them once that load truly finishes -- a real mouse click on a disabled button does nothing, but the test's `RaiseEvent()` call bypasses `IsEnabled` entirely, so it fired Refresh's real, synchronous, CIM/`powercfg.exe`-calling handler while the earlier background load (triggered moments before by the "Windows Setup" nav click, still in flight) was likely still running -- a race no real user can ever trigger, since the UI's own disabled-button state already prevents it. Removed that one test step rather than the tested code; every other click this test exercises remains a faithful simulation of what a real, enabled button click does.
- [x] No behavior, XAML element names, or function signatures changed anywhere in this refactor -- it is a pure code-motion exercise (plus the one real dot-source fix above, which restores the pre-refactor behavior rather than changing it), so no other `TESTING.md` checklist entries needed updating.
