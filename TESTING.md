...
- [x] **Root cause confirmed**: the same diagnostic command succeeded when run from a normal, non-elevated PowerShell window on that same device, account, and moment in time -- it only failed when run elevated. This matches a documented, real limitation of AppX/MSIX-packaged apps (App Installer is one): source/package data can be scoped in a way that an elevated (Administrator-token) process cannot see, even though the exact same user account can see it perfectly fine unelevated. Since this deployment tool always requests elevation (`Start.ps1 -Gui` triggers UAC), **any application's WinGet availability check could report a false "Not Found" on a fresh device that has never had winget's source data initialized under an elevated context** -- this is not specific to Chrome or to this tool's code; it is a Windows/winget platform behavior.
- [x] Decided against a code-level workaround: reliably working around AppX integrity-level scoping from inside an already-elevated PowerShell script is nontrivial and not confidently fixable without introducing real complexity (e.g. shelling out to a de-elevated process), so this is documented as a known device-prep step instead.
- [ ] **Recommended workaround for technicians**: on a genuinely fresh device (one that has never run `winget` before), run any winget command once in a normal, non-elevated PowerShell or terminal window (e.g. `winget --version` or `winget source update`) *before* running this deployment tool's elevated install flow, so winget's source data gets initialized first. Not yet confirmed whether this priming step alone is sufficient to prevent the elevated failure on a subsequent fresh device -- the only device that hit this so far was fixed by testing non-elevated, not by proactively priming beforehand.
